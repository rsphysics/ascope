#!/bin/bash
# ascope_recorder.sh - used as a short way to use usb microscopes.
...
OUTSTRING="$OUTSTRING -nosound -really-quiet -ovc lavc -lavcopts threads=2 "
OUTSTRING="$OUTSTRING $DEVICEIN"

OUTPUTFILE="`date +%F_%H%M%S`.avi"

### START MPLAYER
#echo "DEBUG:OUTSTRING=$OUTSTRING"
mencoder $OUTSTRING -o - | tee $OUTPUTFILE | mplayer -
